...
脚本只使用 Python 标准库。它固定 JSON 字段、白纸墨字版式、10 格推荐条、结论概述单框三段和免责声明。
...
- 结论概述单框三段顺序固定为：最终结论、量化分析、适用场景，渲染成一段话，不再是并列卡片，也不再单设“使用建议”卡片。
...
- 量化分析只报最决定推荐分的 1—2 个关键数字；适用场景只用一句话写清适合谁、做什么，禁止用“一句话：”开头。
- `advice` 字段现在是一句话字符串，不再单独渲染成卡片，只用来和最终结论拼成“这个 Skill 适合你吗”顶部的匹配结论；有条件推荐必须在这句话开头写出具体条件。
- “用户需求、任务步骤、额外需要、最后交付”四张卡全部使用短句列表，2×2 排列，每条不超过 35 个汉字。
